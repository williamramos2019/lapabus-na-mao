-- Corrigir e inserir dados reais das linhas de ônibus
...
-- Inserir horários corrigidos (usando CAST para converter texto para TIME)
INSERT INTO public.bus_schedules (line_id, departure_time, direction, day_type, stop_name, stop_order) 
SELECT 
  bl.id,
  CAST(times.departure_time AS TIME),
  CASE WHEN ROW_NUMBER() OVER (PARTITION BY bl.id ORDER BY times.departure_time) % 2 = 1 THEN 'ida' ELSE 'volta' END AS direction,
  'weekday' AS day_type,
  CASE WHEN ROW_NUMBER() OVER (PARTITION BY bl.id ORDER BY times.departure_time) % 2 = 1 THEN bl.origin ELSE bl.destination END AS stop_name,
  ROW_NUMBER() OVER (PARTITION BY bl.id ORDER BY times.departure_time) AS stop_order
FROM public.bus_lines bl
CROSS JOIN (
  SELECT unnest(ARRAY['05:00', '05:30', '06:00', '06:30', '07:00', '07:30', '08:00', '08:30', '09:00', '09:30', '10:00', '10:30', '11:00', '11:30', '12:00', '12:30', '13:00', '13:30', '14:00', '14:30', '15:00', '15:30', '16:00', '16:30', '17:00', '17:30', '18:00', '18:30', '19:00', '19:30', '20:00', '20:30', '21:00', '21:30', '22:00']) AS departure_time
) times
WHERE bl.line_number IN ('100', '130', '160', '5110', '5140', '5180', '5291', '5297');

-- Inserir horários para sábados
INSERT INTO public.bus_schedules (line_id, departure_time, direction, day_type, stop_name, stop_order) 
SELECT 
  bl.id,
  CAST(times.departure_time AS TIME),
...
FROM public.bus_lines bl
CROSS JOIN (
  SELECT unnest(ARRAY['06:00', '07:00', '08:00', '09:00', '10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00', '17:00', '18:00', '19:00', '20:00', '21:00', '22:00']) AS departure_time
) times
WHERE bl.line_number IN ('100', '130', '160', '5110', '5140', '5180', '5291', '5297');

-- Inserir horários para domingos
INSERT INTO public.bus_schedules (line_id, departure_time, direction, day_type, stop_name, stop_order) 
SELECT 
  bl.id,
  CAST(times.departure_time AS TIME),
...
FROM public.bus_lines bl
CROSS JOIN (
  SELECT unnest(ARRAY['07:00', '09:00', '11:00', '13:00', '15:00', '17:00', '19:00', '21:00']) AS departure_time
) times
WHERE bl.line_number IN ('100', '130', '160', '5110', '5140', '5180', '5291', '5297');